...
export class StorageService {
  ...
  async listFiles(
    ...
    const { bucket } = this.getBucketConfig(bucketName);
  ...
  async isFileExist(fileKey: string, bucketName?: string) {
    try {
      const { bucket } = this.getBucketConfig(bucketName);
  ...
  async getPresignedUrl(
    ...
    if (accessType == AccessType.GET) {
      const fileExist = await this.isFileExist(key, bucketName);
      if (!fileExist) {
  ...
  async uploadFile(
    ...
    const { bucket, publicDomain } = this.getBucketConfig(bucketName);
  ...
  async deleteFolder(prefix: string, bucketName?: string): Promise<void> {
    const { files } = await this.listFiles(prefix, null, bucketName);

    await Promise.all([
      ...files.map((file) => this.deleteFile(file.key, bucketName)),
      this.deleteFile(prefix, bucketName),
    ]);
  }

  private getBucketConfig(bucketName?: string): {
    bucket: string;
    publicDomain: string;
  } {
